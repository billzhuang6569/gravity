...
// 下载视频
async function downloadVideo(taskId: string, url: string, format: string, type: string): Promise<string> {
  return new Promise((resolve, reject) => {
...
app.post('/api/tasks', async (req: any, res: any) => {
  try {
         const { url, format = 'best', type = 'video' } = req.body as { url: string; format?: string; type?: string };
    ...
    (async () => {
      try {
        ...
        const videoInfo = await getVideoInfo(url);
                 ...
                 // 更新任务标题
         if (taskIndex !== -1) {
           tasks[taskIndex].title = (videoInfo.title as string) || `Video from ${url}`;
           tasks[taskIndex].progress = 30;
           tasks[taskIndex].updated_at = new Date().toISOString();
         }

                 // 下载视频
         const fileName = await downloadVideo(taskId, url, format || 'best', type || 'video');
        ...
        if (taskIndex !== -1) {
          ...
          tasks[taskIndex].error = error instanceof Error ? error.message : 'Unknown error';
          tasks[taskIndex].updated_at = new Date().toISOString();
        }
      }
    })();
